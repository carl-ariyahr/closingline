...
//   • candidates: code-card picks for today with status 'play' (gates and spread cap already applied), not started, latest read ok
...
export function buildDailyCard(live, day, now = new Date(), cap = CAP) {
  ...
  chosen.forEach((lp, i) => {
    const rank = base + i + 1;
    lp.dailyCard = { day, rank, builtAt: ts, D: lp.D ?? null }; if (!lp.playsShownAt) lp.playsShownAt = ts;
    card.picks.push({ srcKey: lp.srcKey, rank, pick: lp.pick, game: lp.game, type: lp.type, sport: lp.sport, start: lp.start || null, D: lp.D ?? null, T: lp.T ?? null, H: lp.H ?? null, addedAt: ts });
  });
...
export function cardText(card) {
  const head = `🃏 Card for ${PT.format(new Date(card.day + 'T12:00:00-07:00'))} — ${card.picks.length} play${card.picks.length === 1 ? '' : 's'}${card.added && card.added < card.picks.length ? ` (${card.added} new)` : ''} (top ${card.cap} by gap, ${card.candidates} qualifier${card.candidates === 1 ? '' : 's'} so far, updated ${PTT.format(new Date(card.updatedAt || card.builtAt))} PT)`;
