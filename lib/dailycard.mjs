...
export function buildDailyCard(live, day, now = new Date(), cap = CAP) {
  ...
  chosen.forEach((lp, i) => {
    ...
    lp.dailyCard = { day, rank, builtAt: ts, D: lp.D ?? null, boxes: { n: bx.n, total: bx.total, got: bx.got, miss: bx.miss, ratio: bx.ratio } }; if (!lp.playsShownAt) lp.playsShownAt = ts;
...
export function cardText(card) {
  ...
  const cons = (card.consensus || []).map(p => `🔁 ${p.rank}. ${p.pick} — ${String(p.game).replace(/\s+—.*$/, '')} (${p.sport}${p.start ? ', ' + PTT.format(new Date(p.start)) + ' PT' : ''}) · CONSENSUS, follow the money: crowd ${p.T}% tickets / ${p.H}% money · was our fade ${p.fromFade}`);
  return [head, ...lines, ...cons].join('\n');
}
