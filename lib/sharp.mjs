...
// Carl 2026-09-05: alert on EARLY sharp moves — more than EARLY_HOURS before kickoff (ideally ~6 days out).
export const EARLY_HOURS = 96;
// One paper entry per game+market+side on the sharp-moves card; repeats update the note. Pre-game only.
// Returns {created, updated, alerts: [{pick, game, from, to, tix, hoursBefore, daysBefore}]} — one alert per NEW early move.
export function upsertSharpPicks(card, history, now, opts = {}) {
  const r = { created: 0, updated: 0, alerts: [] };
  for (const [gamecode, h] of Object.entries(history)) {
    ...
    for (const [k, list] of Object.entries(bySide)) {
      ...
      const early = hrs != null && hrs > EARLY_HOURS;
      const days = hrs != null ? Math.round(hrs / 24 * 10) / 10 : null;
      const signal = `${early ? `⚡ EARLY MOVE (${days} days out) · ` : ''}SHARP MOVE (reverse line move): the ${e.market.toLowerCase()} went ${path} toward ${team || e.side.toUpperCase()} while only ${e.tix}% of tickets were on that side — big money on the unpopular side · ${list.length} move${list.length > 1 ? 's' : ''}, last ${PT.format(new Date(e.ts))} PT${hrs != null ? `, ${hrs}h before kickoff` : ''} · paper: tail the move`;
      const alert = { pick: label, game: `${h.away} @ ${h.home} (${h.sport}, ${h.date})`, market: e.market, from: e.from, to: e.to, tix: e.tix, hoursBefore: hrs, daysBefore: days };
      ...
      if (!lp) {
        lp = { kind: 'sharp', src: 'code', sharpKey: key, type: e.market === 'Moneyline' ? 'Moneyline' : e.market, pick: label, game: `${h.away} @ ${h.home} — ${h.date} (${h.sport})`, signal, status: 'watch',
          ...
          away: h.away, home: h.home, date: h.date, sport: h.sport, gamecode, start: h.start || null, createdAt: now.toISOString(), early };
        card.picks.push(lp); r.created++;
        if (early) { lp.alertedFor = e.ts; r.alerts.push(alert); }
      } else if (!lp.result && (lp.signal !== signal || lp.pick !== label)) {
        const isNewMove = lp.sharp?.lastAt !== e.ts;
        lp.signal = signal; lp.pick = label; lp.line = e.market === 'Spread' ? fmt(lineNow) : lp.line; lp.total = e.market === 'Total' ? e.to : lp.total;
        lp.sharp = { ...lp.sharp, to: e.to, moves: list.length, lastAt: e.ts, tix: e.tix, hoursBefore: hrs }; lp.start = h.start || lp.start || null; lp.early = lp.early || early; r.updated++;
        if (early && isNewMove && lp.alertedFor !== e.ts) { lp.alertedFor = e.ts; r.alerts.push(alert); }
      }
    }
  }
  return r;
...
export function stampSharpMoves(picks, history) {
  ...
  for (const lp of picks) {
    ...
    const hrs = hit.length && h.start ? Math.max(0, Math.round((new Date(h.start) - new Date(hit[0].ts)) / 3600e3)) : null; // hours before kickoff of the FIRST move our way
    const early = hrs != null && hrs > EARLY_HOURS;
    if ((lp.sharpMove || null) !== note || (lp.sharpHours ?? null) !== hrs || !!lp.sharpEarly !== early) { lp.sharpMove = note; lp.sharpHours = hrs; lp.sharpEarly = early; changed++; }
  }
  return changed;
}
