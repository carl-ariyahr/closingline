...
// A card is pending when it has plays and was never pushed, or gained plays since the last push (updatedAt > pushedAt).
export function pendingCard(live) {
  for (const card of Object.values(live?.dailyCards || {})) if ((card?.picks?.length || card?.consensus?.length) && (!card.pushedAt || (card.updatedAt && card.updatedAt > card.pushedAt))) return card;
  return null;
}
export function markCardPushed(live, card, now = new Date()) {
  const c = live?.dailyCards?.[card?.day]; if (!c) return 0;
