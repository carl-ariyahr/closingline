...
export function matchGame(events, pick) {
  ...
  if (!cands.length) { // live-card pro format "SF Giants" / "ATH Athletics": drop the ABBR, match the nickname uniquely
    const nickOf = s => String(s || '').trim().split(/\s+/).pop().toLowerCase().replace(/[^a-z]/g, '');
    const abbr = s => String(s || '').replace(/^[A-Z]{2,4}\s+/, '');
    const hitN = (e, k, ours) => teamTail(abbr(ours), e[k]) || sameTeam(e[k], abbr(ours)) || (nickOf(e[k]) === nickOf(abbr(ours)) && nickOf(e[k]) !== 'sox');
    cands = events.filter(e => hitN(e, 'away', pick.away) && hitN(e, 'home', pick.home));
    if (!cands.length) cands = events.filter(e => hitN(e, 'away', pick.home) && hitN(e, 'home', pick.away));
  }
