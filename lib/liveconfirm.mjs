...
export function cohortsOf(lp) {
  const out = [];
  let T = lp.liveCheck?.T ?? lp.T;
  if (T == null) { const m = String(lp.signal || '').match(/(?:PUBLIC|public)\s+(?:on\s+[^—·]*?—\s*)?(\d{1,3})%\s+(?:of\s+)?tickets/i) || String(lp.signal || '').match(/(\d{1,3})%\s+tickets/i); if (m) T = Number(m[1]); }
