...
// DO NOT BET (Carl 2026-09-04): the read at kickoff decides. A pick whose last pre-game read says the crowd FLIPPED
// to our side was not a bet; grading stamps noBet so the record leaves it out. Faded (still the right side, weaker
// gap) is NOT a no-bet — Carl chose "flipped only".
export function noBetAtKickoff(lp) {
  const L = lp.liveCheck;
  return !!(L && !L.ok && L.why === 'flipped');
}
