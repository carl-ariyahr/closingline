...
import { evalGame, gated, gateReasons } from './formula.mjs';
...
// Every VSiN game a live pick could mean (either orientation — live strings have been swapped before).
export function liveCandidates(games, lp) {
  ...
  return straight.length ? straight : pool.filter(x => teamIs(g.away, x.home, x.away) && teamIs(g.home, x.away, x.home));
}
// Resolve to ONE game. A doubleheader (same pair twice on the board) resolves by the pick's own dhIndex,
// else — for a Total — by the number the pick names when exactly one game carries that line. Otherwise
// null: never guess which game of a doubleheader a pick means.
export function matchLiveGame(games, lp) {
  const c = liveCandidates(games, lp);
  if (c.length === 1) return c[0];
  if (c.length < 2) return null;
  if (lp.dhIndex != null) return c.find(x => (x.dhIndex || 0) === lp.dhIndex) || null;
  if (/total/i.test(lp.type)) {
    const num = parseFloat((String(lp.pick).match(/\d+(?:\.\d+)?/) || [])[0]);
    const hit = c.filter(x => x.total?.line === num);
    if (hit.length === 1) return hit[0];
  }
  return null;
}
// Doubleheader warning tag on a live pick that names neither game — one replaceable tag. Returns true if changed.
export const DH_TAG = '⚠ DOUBLEHEADER: two games on the board and this pick does not say which — code cannot re-confirm or grade it; add G1/G2 to the pick';
export function applyDHTag(lp, on) {
  const cleaned = String(lp.signal || '').replace(/\s*·\s*⚠ DOUBLEHEADER[^·]*/g, '').trim();
  const next = on ? `${cleaned} · ${DH_TAG}` : cleaned;
  const changed = next !== String(lp.signal || '');
  lp.signal = next;
  return changed;
}
...
export function liveCheck(lp, game, now = new Date()) {
  ...
  if (fresh) {
    if (fresh.side === side) {
      // Carl 2026-09-07: a read that trips a gate (money under 55% on our side, Over fade, moneyline favorite) is a fade
      if (gated(fresh)) return { ts, ok: false, why: 'faded', gate: gateReasons(fresh), T: fresh.T, H: fresh.H, D: fresh.D };
      return { ts, ok: true, T: fresh.T, H: fresh.H, D: fresh.D, tier: fresh.tier, flag98: !!fresh.flag98 };
    }
...
const GATE_TEXT = { 'money<55': 'money under 55% on our side', 'over-fade': 'Over fade', 'ml-favorite': 'moneyline favorite' };
const gateText = gates => gates.map(g => GATE_TEXT[g] || g).join(', ');
...
export function applyLiveCheck(lp, check, now = new Date()) {
  ...
  const tag = check.ok
    ...
    : check.why === 'flipped'
      ...
      : check.why === 'faded'
        ? `⚠ signal faded on the ${stamp} read (${check.T}% tickets / ${check.H}% money, gap ${check.D} — ${check.gate?.length ? gateText(check.gate) : 'below the 60/8 bar'}). Not a bet unless it comes back before kickoff.`
...
// DO NOT BET: the read at kickoff decides. A pick whose last pre-game read says the crowd FLIPPED to our side
// (Carl 2026-09-04) or whose signal had FADED (Carl 2026-09-07: plays still qualifying at kickoff went 56%, faded
// ones 30%) was not a bet; grading stamps noBet so the record leaves it out.
export function noBetAtKickoff(lp) {
  const L = lp.liveCheck;
  return !!(L && !L.ok && (L.why === 'flipped' || L.why === 'faded'));
}

// Tracked cohorts (Carl 2026-09-05): 'crowd70' = the crowd held >= 70% of tickets on the side we fade (latest read,
// else the posting read); 'dogml' = a moneyline pick on the underdog (plus price). Stamped at grading for the Record tab.
// 'thinmoney' (Carl 2026-09-05: "high tickets but the money is less than 55% on our side") = the crowd side still holds
// more than 45% of the money, so the sharp side of the split is weak.
export function cohortsOf(lp) {
  const out = [];
  if (lp.kind === 'consensus') return out; // follow-the-money plays: the fade cohorts do not apply
  let T = lp.liveCheck?.T ?? lp.T, H = lp.liveCheck?.H ?? lp.H;
  if (T == null || H == null) { const m = String(lp.signal || '').match(/(\d{1,3})%\s+(?:of\s+)?tickets[^·]*?(\d{1,3})%\s+(?:of\s+the\s+)?money/i); if (m) { if (T == null) T = Number(m[1]); if (H == null) H = Number(m[2]); } }
  if (T != null && T >= 70) out.push('crowd70');
  if (H != null && (100 - H) < 55) out.push('thinmoney');
  const ml = /money/i.test(lp.type || '') || /\bML\b/.test(String(lp.pick || ''));
  if (ml) { const price = lp.line != null ? parseFloat(String(lp.line)) : parseFloat((String(lp.pick || '').match(/([+-]\d{3,4})\s*$/) || [])[1]); if (Number.isFinite(price) && price > 100) out.push('dogml'); } // +100 is a pick-em, not a dog
  return out;
}
...
export function liveGradePick(lp, g) {
  ...
  const base = { type, pick: lp.pick, sport: g.sport, date: g.date, away: g.away, home: g.home, dhIndex: lp.dhIndex ?? null }; // null = unknown → grading never guesses a doubleheader game
  if (type === 'Total') {
