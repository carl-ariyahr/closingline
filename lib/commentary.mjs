...
export function cardContext(live, history, day) {
  ...
  return { day, builtAt: card.builtAt, cap: card.cap, candidates: card.candidates, pushedAt: card.pushedAt || null, hasCommentary: plays.every(p => p.commentary), plays };
}
...
export function journalContext(live, journal, day) {
  ...
  const today = all.filter(p => p.date === day && p.dailyCard).map(slim);
  const byRank = {}; for (const p of counted.filter(p => p.dailyCard)) (byRank[p.dailyCard.rank] = byRank[p.dailyCard.rank] || []).push(p);
  ...
  return {
    ...
    ledgerSince: '2026-09-02', ledgerRecord: rec(counted), cardEraRecord: rec(counted.filter(p => p.dailyCard)),
