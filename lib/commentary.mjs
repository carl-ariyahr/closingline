...
const slim = lp => ({
  srcKey: lp.srcKey, rank: lp.dailyCard?.rank ?? null, kind: lp.kind || 'fade', fromFade: lp.fromFade || null, pick: lp.pick, type: lp.type, side: lp.side, line: lp.line ?? null, total: lp.total ?? null,
...
export function cardContext(live, history, day, { onlyMissing = false } = {}) {
  ...
  const plays = [...card.picks, ...(card.consensus || [])].map(cp => {
...
export function pendingCommentaryDay(live) {
  ...
  for (const day of days) { const c = live.dailyCards[day]; if ([...(c.picks || []), ...(c.consensus || [])].some(cp => !byKey.get(cp.srcKey)?.commentary)) return day; }
  return null;
}
...
export function applyCommentary(live, day, map, now = new Date(), maxLen = 600) {
  ...
  const keys = new Set([...card.picks, ...(card.consensus || [])].map(p => p.srcKey));
  let n = 0;
