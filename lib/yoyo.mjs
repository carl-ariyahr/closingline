...
export function upsertYoYoPicks(card, history, now, opts = {}) {
  ...
  for (const [gamecode, h] of Object.entries(history)) {
    ...
    for (const mk of ['Spread', 'Total']) {
      ...
      const signal = `Yo-yo line (VSiN/DK ${mk.toLowerCase()}): ${mk === 'Spread' ? path : yy.path.join(' → ')} — left ${fmt(yy.from)} ${PT.format(new Date(yy.ts[1]))}, back to ${fmt(yy.back)} ${PT.format(new Date(yy.ts[2]))} PT · flag only, not a bet`;
      if (!lp) {
