...
// American odds on a linear "cents" scale so -105 → +105 is a 10-cent move, not 210: -110 → -10, +150 → +50
export const cents = p => (p >= 100 ? p - 100 : p <= -100 ? p + 100 : 0);
// minimum move for a change to count: any change for spreads/totals, 20 cents for moneylines (Carl 2026-09-04)
const MIN_STEP = { Spread: 0, Total: 0, Moneyline: 20 };
const scale = mk => (mk === 'Moneyline' ? cents : v => v);
...
export function recordLines(history, games, ts) {
  ...
  for (const g of games) {
    ...
    h.excluded = !!g.excluded; // FCS-vs-FCS (Carl's college scope) — kept in history, never surfaced
    for (const [mk, read] of Object.entries(MARKETS)) {
      ...
      const xf = scale(mk);
      const moved = last === null ? true : MIN_STEP[mk] > 0 ? Math.abs(xf(Number(v)) - xf(Number(last))) >= MIN_STEP[mk] : Number(v) !== Number(last);
      // tickets on the away/over side at this read; a change point remembers the tickets BEFORE the move (sharp-move detection)
      const bets = mk === 'Spread' ? g.spread?.away?.bets : mk === 'Total' ? g.total?.over?.bets : g.ml?.away?.bets;
      h.tix = h.tix || {};
      if (moved) { arr.push([ts, Number(v), h.tix[mk] ?? (bets ?? null)]); n++; }
      if (bets != null) h.tix[mk] = bets;
    }
  }
  return n;
}
...
export function detectYoYo(series, xf = v => v) {
  if (!series || series.length < 3) return null;
  const v0 = xf(series[0][1]);
  let far = 0; // index of the furthest excursion so far
  for (let k = 1; k < series.length; k++) {
    const dPrev = Math.abs(xf(series[k - 1][1]) - v0), dNow = Math.abs(xf(series[k][1]) - v0);
    if (dNow < dPrev && dPrev > 0) {
      return { from: series[0][1], to: series[far][1], back: series[k][1], path: series.slice(0, k + 1).map(x => x[1]), ts: [series[0][0], series[far][0], series[k][0]] };
    }
    if (dNow > Math.abs(xf(series[far][1]) - v0)) far = k;
  }
  return null;
}
...
export function upsertYoYoPicks(card, history, now, opts = {}) {
  ...
  for (const [gamecode, h] of Object.entries(history)) {
    ...
    for (const mk of ['Spread', 'Total', 'Moneyline']) {
      const yy = detectYoYo(h[mk], scale(mk));
      if (!yy) continue;
