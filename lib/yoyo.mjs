// Yo-yo line detection (Carl 2026-09-04, Patrick's Variables only): a line that goes from one number to a different
// number and then BACK to the original number — e.g. Niners -3 → -6 → -3. Any size of move counts (Carl's call).
// Markets: spreads (home line) and totals. MLB/NHL moneyline prices are not watched (no spread pick in those sports).
...
const MARKETS = { Spread: g => g.spread?.line_home ?? null, Total: g => g.total?.line ?? null };
const SPREAD_SPORTS = new Set(['NFL', 'CFB', 'NBA', 'CBB', 'WNBA']);
...
export function recordLines(history, games, ts) {
  ...
  for (const g of games) {
    ...
    const h = history[g.gamecode] || (history[g.gamecode] = { sport: g.sport, date: g.date, away: g.away, home: g.home, dhIndex: g.dhIndex || 0, Spread: [], Total: [] });
    h.start = g.start || h.start || null;
    for (const [mk, read] of Object.entries(MARKETS)) {
      if (mk === 'Spread' && !SPREAD_SPORTS.has(g.sport)) continue;
      const v = read(g);
...
// series: [[ts, value], ...] change points. Returns null or the FIRST yo-yo: {from, to, back, path, ts:[tFrom, tTo, tBack]}
export function detectYoYo(series) {
  if (!series || series.length < 3) return null;
  for (let k = 2; k < series.length; k++) {
    const vk = series[k][1];
    for (let i = 0; i < k - 1; i++) {
      if (series[i][1] !== vk) continue;
      // furthest excursion between i and k
      let j = i + 1;
      for (let m = i + 1; m < k; m++) if (Math.abs(series[m][1] - vk) > Math.abs(series[j][1] - vk)) j = m;
      return { from: vk, to: series[j][1], back: vk, path: series.slice(i, k + 1).map(x => x[1]), ts: [series[i][0], series[j][0], series[k][0]] };
    }
  }
  return null;
}
...
export function upsertYoYoPicks(card, history, now, opts = {}) {
  ...
  for (const [gamecode, h] of Object.entries(history)) {
    ...
    for (const mk of ['Spread', 'Total']) {
      ...
      const label = mk === 'Spread' ? `${h.home} spread ${path}` : `Total ${yy.path.join(' → ')}`;
      const key = `yoyo|${gamecode}|${mk}`;
      let lp = card.picks.find(x => x.yoyoKey === key);
      const f = mk === 'Spread' ? fmt : String;
      const signal = `Yo-yo line (VSiN/DK ${mk.toLowerCase()}): ${yy.path.map(f).join(' → ')} — left ${f(yy.from)} ${PT.format(new Date(yy.ts[1]))}, back to ${f(yy.back)} ${PT.format(new Date(yy.ts[2]))} PT · flag only, not a bet`;
      if (!lp) {
